...
from data_utils.collate import collate_fn_eval_baseline
from utils.log import print_results
from data_utils.data_class import DSEC11_CLASS, DSEC11_INSTANCE_ID, DSEC11_PRED_TO_ID, DSEC19_CLASS, DSEC19_INSTANCE_ID, DSEC19_PRED_TO_ID, DDD17_CLASS, DDD17_INSTANCE_ID, DDD17_PRED_TO_ID
    ...
    for pred, gt in zip(predictions, ground_truths):
        ...
        if pred['label'] == cls:
            if gt['label'] == cls:
                ...
                score = 1.0 if iou >= iou_threshold else 0.0 # set confidence score to 1.0
            else:
                score = 1.0
        else:
            score = 0.0
...
def test(ctx: DictConfig):
    ...
    if "frame" in ctx.model.type:
        baseline.load_state_dict(torch.load(ctx.general.checkpoint, weights_only=True), strict=False)
    ...
    for event_representation, evimg, semantic_label, instance_mask, instance_label in tqdm(BaselineLoader):
        if ctx.model.type in ['image_crop']:
        ...
        for mask, label in zip(instance_mask, instance_label):
            with torch.no_grad():
                if ctx.dataset.test.prompt == "box":
                ...
                elif ctx.dataset.test.prompt == "point":
