...
from models.snn.spikes import *
from models.snn.net_utils import AverageMeterNetwork
...
class SNN_VGG(nn.Module):
    def __init__(
        ...
        self.leak_mem = torch.tensor(leak_mem)
        self.def_threshold = def_threshold
        self.upsample_threshold = def_threshold
        self.thl = thl
    ...
    def _make_layers(self):
        ...
        for x in (cfg_features[self.architecture]):
            if isinstance(x, str) and x[:3] == 'avg':
                ...
                self.skips_features[str(layer)] = nn.Conv2d(in_channels, self.out_channels, kernel_size=1)
                self.skips_counter += 1
                continue
            else:
        ...
        for x in (cfg_classifier[self.architecture]):
            if isinstance(x, str) and x[:3] == 'avg':
                ...
                self.skips_classifier[str(layer)] = nn.Conv2d(in_channels, self.out_channels, kernel_size=1)
                self.skips_counter += 1
                continue
            elif isinstance(x, str) and x == 'output':
                layers += [nn.Conv2d(in_channels, self.out_channels, kernel_size=1, padding=0, stride=stride, bias=bias_flag)]
                continue
            elif isinstance(x, str) and x[:6] == 'atrous':
        ...
        if self.fcn:
            ...
            for _ in range(self.skips_counter):
                ...
                layers += [nn.Sequential(
                    nn.ConvTranspose2d(self.out_channels, self.out_channels, kernel_size=self.kernel_size, stride=2, bias=False),
    ...
    def _initialize_mems(self, N, layer=None, get_layer_shape=False):
        ...
        if self.fcn:
            ...
            for _ in range(self.skips_counter):
                divisor = divisor//2
                if isinstance(x, int):
                    layers += [torch.zeros(N, self.out_channels, height//divisor, width//divisor).cuda()] if self.gpu else [torch.zeros(N, self.out_channels, height//divisor, width//divisor)]
                    if get_layer_shape and i == layer:
                        return layers[-1].shape
                    i += 1
            mem_upsample = layers
